...
def compute_metrics(logs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "json_generation_rate": (
            (json_generated / schema_requests * 100) if schema_requests > 0 else 0
        ),
...
def main():
    """Main dashboard app."""
    st.set_page_config(page_title="LLM Gateway Dashboard", page_icon="📊", layout="wide")
...
python -m uvicorn src.llmops.gateway:app --host 127.0.0.1 --port 8000
    ...
    if "token_usage" in df.columns:
        ...
        if not token_df.empty and "timestamp" in token_df.columns:
            token_df["timestamp"] = pd.to_datetime(
                token_df["timestamp"], format="ISO8601", utc=True
            )
    ...
    if "rate_limited" in df.columns:
        ...
        rate_limit_pct = (
            (rate_limited_total / metrics["total_requests"] * 100)
